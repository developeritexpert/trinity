...
// Pleat styles
const pleatStyles = {
    pleats_single: "pleats_single.png",
    pleats_double: "pleats_double.png",
};
...
// URL builder
function buildPleatURL(fabricKey, pleatFile) {
    return `${BASE}/${fabricKey}/front/${pleatFile}`;
}

// ================= MAIN =================

async function run() {
    console.log("🚀 Downloading trouser pleats...\n");
    ...
    for (const [fabricKey, fabricName] of Object.entries(fabrics)) {
        ...
        for (const [fileName, pleatFile] of Object.entries(pleatStyles)) {
            ...
            const url = buildPleatURL(fabricKey, pleatFile);
        ...
        if (successCount === 0) {
            console.log(`⚠️ No new pleat assets downloaded for ${fabricName}`);
        } else {
            console.log(`🎉 ${successCount}/2 pleat images downloaded`);
        }
    }

    console.log("\n✅ All pleat downloads completed!");
}
